...
def idx_to_bin_img(img,idx,gauss=None):
  ...
  for j in range(0,len(idx)):
    img[idx[j,1],idx[j,0]] = 255
  if gauss is not None:
    img = np.asarray(filters.gaussian(
          np.asarray(img,dtype=float), gauss),dtype=np.uint8)
  return img
...
def scores(N=100):
  ...
  # read the data
  imgx = np.zeros((100,500,500),dtype=np.uint8)
  imgy = np.zeros((100,500,500),dtype=np.uint8)
  for i in range(0,N):
    # inputs
    imgrgb = imread(imgslug.replace('#',str(i+1)))
    imgx[i,:,:] = imgrgb[:,:,0] # R channel
    # labels
    nucidx = np.floor(loadmat(matslug.replace('#',str(i+1)))['detection'])
    imgy[i,:,:] = idx_to_bin_img(imgy[i,:,:], nucidx, gauss=2)
    # debug
